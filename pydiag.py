...
def main(argv):
   ...
   def read_params(param_file):
      ...
      return cluster_file,mock_file,c_id_col,cg_id_col,cg_z_col,m_id_col,mg_id_col,mg_z_col,m_mass_col,match_threshold,rich_bin_size,max_rich_bin,min_rich_bin,z_bin_size
   ...
   try:
      opts, args = getopt.getopt(argv,"c:m:p:a:b:c:d:e:f:g:h:i:j:k:l:z",["cfile=","hfile=","pfile=","c_id_col=","cg_id_col=","cg_z_col="
                                                               "m_id_col=","mg_id_col=","mg_z_col=","m_mass_col=","binsize=",
         "match_threshold=","rich_bin=","max_rich_bin=","min_rich_bin=","z_bin_size=","help"])
   ...
   for opt, arg in opts:
      if opt in ('-h','--help'):
         ...
         param_file = arg
         cluster_file,mock_file,c_id_col,cg_id_col,cg_z_col,m_id_col,mg_id_col,mg_z_col,m_mass_col,match_threshold,rich_bin_size,max_rich_bin,min_rich_bin,z_bin_size = read_params(param_file)
   ...
   m_one_way = np.zeros(len(m_id_u_3))                                                    #flag cluster if it satisfies 1-way macth
   m_two_way = np.zeros(len(m_id_u_3))                                                    #flag cluster if it satisfies 2-way macth
   ...
   n_rich_bins = int(math.floor((max(max(c_rich_u), max(m_rich_u_3)) - min(c_rich_u)) / rich_bin_size)) + 1       #set number of richness bins

   x_rich_vals = []
   for i in range(n_rich_bins): x_rich_vals.append(min_rich_bin + rich_bin_size*(i+0.5))  #set richness bin centres
   x_rich_vals = np.array(x_rich_vals)

   c_bin_count = np.zeros(n_rich_bins).astype('int')
   m_bin_count = np.zeros(n_rich_bins).astype('int')
   c_match_bin_count_1 = np.zeros(n_rich_bins).astype('int')
   c_match_bin_count_2 = np.zeros(n_rich_bins).astype('int')
   m_match_bin_count_1 = np.zeros(n_rich_bins).astype('int')
   m_match_bin_count_2 = np.zeros(n_rich_bins).astype('int')
   c_bin_index = np.floor((c_rich_u - min_rich_bin) / rich_bin_size).astype('int')
   m_bin_index = np.floor((m_rich_u_3 - min_rich_bin) / rich_bin_size).astype('int')

   for i in range(len(c_id_u)):                                                          #cluster match count in richness bins
      c_bin_count[c_bin_index[i]] += 1
      if c_one_way[i] == 1:
         c_match_bin_count_1[c_bin_index[i]] += 1
      if c_two_way[i] == 1:
         c_match_bin_count_2[c_bin_index[i]] += 1
           
   for i in range(len(m_id_u_3)):                                                        #halo match count in richness bins
      m_bin_count[m_bin_index[i]] += 1
      if m_id_u_3[i] in m_id_match:
         ...
         for j in range(len(an_index)):
            if c_one_way[an_index[j]] == 1:
               m_match_bin_count_1[m_bin_index[i]] += 1
            if c_two_way[an_index[j]] == 1:
               m_match_bin_count_2[m_bin_index[i]] += 1
   ...
   gs1 = gridspec.GridSpec(4, 1, height_ratios=[1,1,1,1])
   ...
   ax1.plot(x_rich_vals,complete_bin_1,linewidth=1.5,c='b',label='One-Way')                #one-way completeness plot
   ax1.plot(x_rich_vals,complete_bin_2,linewidth=1.5,c='r',label='Two-Way')                #two-way completeness plot
   ax1.set_xlim(min_rich_bin,max_rich_bin)
   ax1.set_ylim(0,1.4)
   ax1.set_ylabel('Completeness')
   ax1.set_title('Cluster Metrics')
   ax1.legend()
   ax1.yaxis.set_major_locator(MaxNLocator(3))
   ax1.xaxis.set_visible(False)

   ax2.plot(xyline,(1,1),'--',c='0.5',linewidth=1.0)
   ax2.plot(x_rich_vals,pure_bin_1,linewidth=1.5,c='b',label='One-Way')                   #one-way purity plot
   ax2.plot(x_rich_vals,pure_bin_2,linewidth=1.5,c='r',label='Two-Way')                   #two-way purity plot
   ax2.set_xlim(min_rich_bin,max_rich_bin)
   ax2.set_ylim(0,1.4)
   ...
   ax3.plot(xyline,(1,1),'--',c='0.5',linewidth=1.0)
   ax3.plot(x_rich_vals,g1_bin,linewidth=1.5,c='b')                                       #g1 parameter plot
   ax3.set_xlim(min_rich_bin,max_rich_bin)
   ax3.set_ylim(0,1.4)
   ax3.set_ylabel('G1')
   ax3.yaxis.set_major_locator(MaxNLocator(3))
   ax3.xaxis.set_visible(False)

   ax4.plot(xyline,(1,1),'--',c='0.5',linewidth=1.0)
   ax4.plot(x_rich_vals,g2_bin,linewidth=1.5,c='b')                                       #g2 parameter plot
   ax4.set_xlim(min_rich_bin,max_rich_bin)
   ax4.set_ylim(0,1.4)
   ax4.set_ylabel('G2')
   ax4.set_xlabel('Richness')
   ax4.yaxis.set_major_locator(MaxNLocator(3))
   
   fig1_name = cluster_file + '_plots.pdf'
   fig1.savefig(fig1_name)
   print "Plots saved to:", fig1_name

   fig2 = plt.figure()
   if z_bin_size < 1: z_bin_size = 0.05
   nz_nbins1 = (max(cg_z)-min(cg_z))/z_bin_size
   nz_nbins2 = (max(mg_z)-min(mg_z))/z_bin_size

   fig2 = plt.figure() 
   plt.hist(cg_z, nz_nbins1, normed=1, facecolor='green', alpha=0.5, label='Cluusters')  #N(z) histogram plot
   plt.hist(mg_z, nz_nbins2, normed=1, facecolor='red', alpha=0.5, label='Haloes')
   plt.legend()
   plt.xlabel('$z$')
   plt.title('N(z) Distribution')

   fig2_name = cluster_file + '_nz.pdf'
