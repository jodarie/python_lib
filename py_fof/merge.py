...
from classes.cluster import Cluster, kdtree
...
def find_matches(clusters, progress, tree_dist):
    ...
    for i in range(len(clusters)):
        ...
        if not clusters[i].match_flag:
            list = tree.query_ball_point([clusters[i].xpos, clusters[i].ypos, clusters[i].zpos], tree_dist)
            for j in list:
...
def merge(cluster1, cluster2):
    ...
    """
    cluster1.extend(cluster2.g_num, cluster2.g_id, cluster2.g_ra, cluster2.g_dec,
                    cluster2.g_z, cluster2.g_xpos, cluster2.g_ypos, cluster2.g_zpos)
