...
def read_mock(opts):
    ...
    mock = np.genfromtxt(opts.input_files[1], dtype = "S",
                         unpack = True)[np.array(opts.mock_cols) - 1]
...
def read_obs(opts):
    ...
    obs = np.genfromtxt(opts.input_files[0], dtype="S",
                        unpack = True)[np.array(opts.obs_cols) - 1]
    
    dtypes = [('id', 'i4'), ('ra', 'f8'), ('dec', 'f8'), ('z', 'f8'),
              ('proxy', 'f8'), ('sn', 'f8')]
    ...
    index = ((obs.z >= opts.z_bin[0]) & (obs.z <= opts.z_bin[1]) &
         (obs.proxy >= opts.proxy_bin[0]) & (obs.proxy <= opts.proxy_bin[1]))
...
def print_matrix(matrix, hm_matrix, ranges, opts):
    ...
    for i in range(len(matrix)):
        ...
        file_name = opts.input_files[0] + '.pycy.matrix.z' + str(ranges[0][i]) + '.txt'

        output = np.vstack([np.append(-1, hm_matrix[i]), np.append(-1, np.sum(matrix[i], axis = 1)),
...
def print_matches(mock, obs, index, weights, opts):
    ...
    print>> output, '#MOCK_ID MOCK_Mass [OBS_ID OBS_PROXY WEIGHT]...'

    for i in range(mock.size):
        print>> output, mock.id[i], mock.mass[i],
        
        if isinstance(index[i], np.ndarray):
            for j in range(len(index[i])):
                    print>> output, [obs.id[index[i][j]], int(10 ** obs.proxy[index[i][j]]), weights[i][j]],
            print>> output, ''
        else:
            print>> output, [obs.id[index[i]], int(10 ** obs.proxy[index[i]]), weights[i]]
