...
def make_plots(matrix, hm_matrix, ranges, opts):
    ...
    for i in range(len(ranges[2])):
        mass_obs(np.flipud(matrix[i]), ranges[2][i], opts, mass_obs_plots)
        n_lambda(matrix[i], hm_matrix, ranges, i, opts, n_lambda_plots)
        mass_func(hm_matrix[i], matrix[i], ranges[1], ranges[2][i], opts, mass_func_plots)
...
def mass_obs(matrix, z_bin, opts, page):
    ...
    extent = [opts.proxy_bin[0], opts.proxy_bin[1], opts.mass_bin[0], opts.mass_bin[1]]
    
    plt.imshow(matrix, extent = extent,
            interpolation = 'spline16', cmap = cm.jet,
            aspect = 'auto', norm = mp.colors.LogNorm())
    
    cbar = plt.colorbar()
    cbar.set_label(r"$n(\lambda|M,z)$")
    
    plt.autoscale(False)
    plt.xlabel(r'$Log_{10} \lambda$', {'fontsize' : 15})
    plt.ylabel(r'$Log_{10} M$ $[M_{\odot} h^{-1}]$', {'fontsize' : 15})
    plt.title(r'z$_i$ = ' + str(z_bin),{'fontsize' : 15})
...
def mass_func(mass_hist, matrix, x_range, z_bin, opts, page):
    ...
    plt.plot(x_range, mass_hist, '-', color = 'blue',
             label = 'Mock Haloes: ' + str(int(np.sum(mass_hist))))
    plt.plot(x_range, matrix.sum(axis = 1), '--', color = 'red',
             label = 'Observed Clusters: ' + str(int(np.sum(matrix))))

    plt.ylabel(r"$N(M)$", {'fontsize' : 15})
    plt.xlabel(r"$Log_{10} M$ $[M_{\odot} h^{-1}]$", {'fontsize' : 15})
    plt.title(r'z$_i$ = ' + str(z_bin),{'fontsize' : 15})

    max_val = np.ceil(np.log10(np.max([np.max(mass_hist), np.max(matrix.sum(axis = 0))])))
...
def plot_complete(matrix, plot_type, opts):
    ...
    if plot_type == 'mass':
        ...
        extent = [opts.z_bin[0], opts.z_bin[1], opts.mass_bin[0], opts.mass_bin[1]]
        ax.set_ylim(opts.mass_bin[0], opts.mass_bin[1])
    elif plot_type == 'ngal':
        ax.set_ylabel('log$_{10}$ N$_{gal}$')
        tag = '.pycy.complete_ngal.pdf'
        extent = [opts.z_bin[0], opts.z_bin[1], opts.proxy_bin[0], opts.proxy_bin[1]]
        ax.set_ylim(opts.proxy_bin[0], opts.proxy_bin[1])

    im = ax.imshow(matrix, extent = extent,
                   aspect = 'auto', interpolation = 'spline16', cmap = colormap,
...
def plot_pure(matrix, opts):
    ...
    ax.set_xlabel('z')
    ax.set_ylabel('log$_{10} \lambda$')

    im = ax.imshow(matrix, extent = [opts.z_bin[0], opts.z_bin[1], opts.proxy_bin[0], opts.proxy_bin[1]],
                   aspect = 'auto', interpolation = 'spline16', cmap = colormap,
                   norm = mp.colors.BoundaryNorm(boundaries, colormap.N))
