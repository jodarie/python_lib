...
def get_opts():
    ...
    o_col_help = ('Set column numbers for cluster properties: ID, RA, DEC, Z, NGAl, SN. ' + \
                '[Default 1 2 3 4 5 6]')

    parser = argparse.ArgumentParser('PYCYMATCH OPTIONS:')
    
    parser.add_argument('-i', '--input_files', dest = 'input_files', nargs = '+',
                        help = 'Input file names. (1-Observerd Clusters, 2-Mock Haloes)')
    
    parser.add_argument('-z', '--delta_z', dest = 'delta_z', default = 0.03, type = float,
                    help = 'Delta_z value. [Default: 0.03]')
    
    parser.add_argument('--proxy_bin', dest = 'proxy_bin', default = [0.0, 3.5, 0.2],
    ...
    parser.add_argument('--mock_plots', action = 'store_true', dest = 'make_mock_plots',
