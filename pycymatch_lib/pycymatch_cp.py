...
def get_completeness(mock, matches, opts):
    ...
    n_ngal_bins = comp.num_bins(opts.proxy_bin[0], opts.proxy_bin[1], opts.proxy_bin[2])
    ...
    ngal_index = np.floor((mock.ngal - opts.proxy_bin[0]) / opts.proxy_bin[2]).astype('int')
